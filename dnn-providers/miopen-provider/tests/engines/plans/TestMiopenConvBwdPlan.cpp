...
TEST(TestConvBwdParams, PadsOneDimensionalGraphToTwoDimensions)
{
    ...
    const std::vector<int64_t> dxDims = {1, 4, 8};
    const std::vector<int64_t> dxStrides = {32, 8, 1};
    ...
    for(const auto* tensor : {&params.dx(), &params.w(), &params.dy()})
    {
        int dimCount = 0;
