...
> **Historical record.** This documents the single-wave WMMA campaign and the ~11 TF
> plateau it reached. A later structural rewrite — computing the scores transposed,
> `Sᵀ = K·Qᵀ` — roughly doubled that and is what ships today, alongside a paged split-K
> decode kernel. For the kernels currently in production and their end-to-end results,
> see [`README.md`](README.md). The analysis below remains valid for the configuration
> it was measured at; several of its verdicts were later invalidated by changes it was
> never re-checked against, which `README.md` §9.3 and §10 discuss.
