...
passed straight to ``rocblas_gemm_ex`` with ``rocblas_gemm_algo_solution_index``. Solutions
provided by the legacy Tensile backend are reported as negative indices (with bias), while positive indices
...
ROCBLAS_USE_HIPBLASLT environment variable can be used to force the use of the hipBLASLt backend and defeat the override file, 
nor will it work when hipBLASLt is the default backend.
