...
belong to the hipBLASLt backend.
...
Because the overrides are applied to the Tensile backend only, entries whose ``solution_index`` does not
identify a Tensile solution are skipped and reported as a load warning. Positive indices in an override
file are still interpreted as raw one-based Tensile indices for compatibility with files generated by
earlier releases. To see which entries were applied, set the environment variable ``TENSILE_DB=0x40000``.
