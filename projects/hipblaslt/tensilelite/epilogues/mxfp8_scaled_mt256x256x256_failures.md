...
| Fix | Effect |
|-----|--------|
| Initial tensilelite fix (commit in branch) | Corrected ~90 % failures across all solutions → 11/13 pass; `MT256x256x256` PGR=0/1 still fail, **PGR=2 now passes** |
| Second fix (applied, rerun pending) | Under investigation — PGR=0/1 still failing as of run 2 |

The progression from ~90 % error rate on all solutions to only PGR=0/1
failing narrows the root cause to a prefetch-path issue specific to
`MT256x256x256 / MIWT4_16 / WG64_4_1` with PGR < 2.
...
was generated from the 11 passing solutions (run 1). The two failing
configurations (`MT256x256x256 / MIWT4_16 / WG64_4_1`, PGR=0 and PGR=1)
are **not** included and will not be selected at runtime. The file will be
regenerated from all 13 solutions once both configurations pass.
