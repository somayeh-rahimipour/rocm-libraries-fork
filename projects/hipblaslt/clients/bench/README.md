...
--uniform_summation_order <value>  Request a uniform summation order across the M dimension via the HIPBLASLT_MATMUL_DESC_UNIFORM_SUMMATION_ORDER_EXT extension attribute. Accepts off|0, on|1, case-insensitive. When on, if every row of A holds the same K-vector then every row of D is bitwise identical within that run. This is uniformity across M inside a single run, NOT run-to-run determinism, and it can cost performance. If the resolved configuration cannot honor the guarantee the matmul returns HIPBLAS_STATUS_INVALID_VALUE instead of silently producing non-uniform output. Omit to leave the attribute unset so the library default (off) applies.  (Default value is: unset)
