// CDNA4 grouped convolution kernel, 32 channels per group (wave-pair model).
//
// This unified TU subsumes the original grouped_32c (fp16/bf16) and
// grouped_32c_tf32 (Phase 1) variants behind a single GroupedDataTraits<DT>
// abstraction (mirroring grouped_16c.cpp / grouped_4c.cpp). The kernel body
// is parameterised by DataType DT; fp16, bf16 and tf32 share the same control
// flow with two structural `if constexpr` branches:
//   1) Dgrad weight prologue: fp16/bf16 use two ds_read_tr16_b64 calls per
//      filter tap to stitch a 16x32 fragment via hardware (k <-> c) transpose;
//      tf32 has no ds_read_tr32 builtin and uses 8 scalar fp32 reads per
//      filter tap per lane (= 72 ds_read_b32 over kh*kw=9 taps in the
//      prologue) to perform the same transpose in software.
...
// used in grouped_4c.cpp / grouped_16c.cpp: the input main loop calls
// fp32x8_to_bf16_pair once per (Y_LOCAL, S) tap so the inner R loop can call
// mfma_16x16x32(bf16_pair_x8, bf16_pair_x8, fp32x4_t) without re-splitting on
// every MFMA.
...
// to mfma_16x16x32, the LDS access unit for output staging, and the swizzle
// layout, plus the to_mfma_operand / zero_operand helpers that absorb the
// fp32 -> bf16-pair split for tf32 while remaining identity for fp16/bf16.
// ----------------------------------------------------------------------------
...
struct GroupedDataTraits<DataType::bf16>
{
    ...
    static __device__ __forceinline__ mfma_operand_t zero_operand()
    {
        return mfma_operand_t{
            bf16_t(0), bf16_t(0), bf16_t(0), bf16_t(0), bf16_t(0), bf16_t(0), bf16_t(0), bf16_t(0)};
    }
};
...
struct Config
{
    ...
    constexpr int waves_per_wg() const { return groups_per_wg * 2; }
...
// Max groups_per_wg = 8; limited by register allocation and LDS.
// groups_per_wg = 2 performs better in practice.
constexpr Config configs[] = {
    {.groups_per_wg = 2, .direction = Direction::Dgrad},
    {.groups_per_wg = 1, .direction = Direction::Dgrad},
    {.groups_per_wg = 2, .dilation = 2, .direction = Direction::Dgrad},
    {.groups_per_wg = 1, .dilation = 2, .direction = Direction::Dgrad},
    {.groups_per_wg = 2},
    {.groups_per_wg = 1},
    {.groups_per_wg = 2, .stride = 2},
    {.groups_per_wg = 1, .stride = 2},
};

constexpr int NUM_CONFIGS = sizeof(configs) / sizeof(configs[0]);

template <Config cfg, DataType DT>
__device__ void conv2d_grouped_32c_cdna4_nhwc_impl(const ToType<DT>* __restrict__ in,
                                                   ...
                                                   int px)
{
    ...
    // MFMA 16x16x32: operands are 8-element fragments per lane, result fp32x4.
    ...
    // Number of uint4 slots per group along C (4 for fp16/bf16, 8 for tf32).
    constexpr int GROUP_UINT4 = GROUP_SIZE / CH_PER_UINT4;
    // Number of uint4 slots covered by one lane's K=8 fragment (1 for fp16/bf16,
    // 2 for tf32 because each fp32 takes twice the storage of a fp16/bf16
    // half).
    constexpr int UINT4_PER_LANE_FRAGMENT = 8 / CH_PER_UINT4;
    ...
    constexpr int BLOCK_GROUPS = cfg.groups_per_wg;
    ...
    constexpr int WEIGHT_LDS_SIZE_UINT4 = BLOCK_GROUPS * GROUP_SIZE * cfg.kh * cfg.kw * GROUP_UINT4;
    ...
    // bf16_pair_x8 for tf32 (caller-side fp32 -> bf16 pair split done in this
    // prologue so the K loop reuses the pre-split operands).
    mfma_operand_t weights_reg[cfg.kh * cfg.kw];
    {
        auto weight_bytes = static_cast<size_t>(groups * GROUP_SIZE) * cfg.kh * cfg.kw *
                            GROUP_SIZE * sizeof(element_t);
        auto weight_rsrc = __builtin_amdgcn_make_buffer_rsrc(
            const_cast<element_t*>(wei), 0, weight_bytes, rsrc_data_format);
        uint32_t voffset_base = block_k * cfg.kh * cfg.kw * GROUP_SIZE * sizeof(element_t);
        ...
        if constexpr(cfg.direction == Direction::Dgrad)
        {
            ...
            {
                // fp16/bf16 path: two ds_read_tr16_b64 calls per filter tap
                // stitch a complete 16x32 MFMA B operand via hardware
                // (k <-> c) transpose.
                //
                // LDS layout is [k_all][kh*kw][c] (straight copy from global).
                // k_stride = kh * kw * GROUP_SIZE (in halves) between output
                // channels.
                const int k_stride = cfg.kh * cfg.kw * GROUP_SIZE;
                auto* wei_grp_base =
                    reinterpret_cast<element_t*>(lds_buf) + wave_group * GROUP_SIZE * k_stride;
                ...
                for(int khw = 0; khw < cfg.kh * cfg.kw; khw++)
                {
                    ...
                    operand_t w_raw = __builtin_bit_cast(
                        operand_t,
                        (int16x8_t){r0[0], r0[1], r0[2], r0[3], r1[0], r1[1], r1[2], r1[3]});
                    weights_reg[khw] = Traits::to_mfma_operand(w_raw);
                }
            }
            else
            {
                // tf32 path: software (k <-> c) transpose. gfx950 has no
                // ds_read_tr32 builtin so each lane reads 8 scalar fp32
                // weights at the transposed (m, k) positions the MFMA A
                // operand expects:
                //   fragment[lane] = w[m=input_c=wave_half*16+lane%16,
                //                      k=output_c=wave_group*32+(lane/16)*8..(lane/16)*8+7]
                // for every (r, s) tap. Costs 8 ds_read_b32 per tap per lane
                // (= 72 ds_read_b32 over kh*kw=9 taps), only run once in
                // prologue.
                const element_t* wei_elem = reinterpret_cast<const element_t*>(lds_buf);
                const int k_stride        = cfg.kh * cfg.kw * GROUP_SIZE;
                const int base_k          = wave_group * GROUP_SIZE;
                const int c_lane          = wave_half * 16 + (lane % 16);
                ...
                for(int khw = 0; khw < cfg.kh * cfg.kw; khw++)
                {
                    ...
                    for(int i = 0; i < 8; i++)
                    {
                        ...
                        int idx      = k_global * k_stride + khw * GROUP_SIZE + c_lane;
        ...
        {
            // Fprop: linear weight read in MFMA operand layout. Each lane
            // reads one operand_t (8 elements) per (k_global, khw) row.
            ...
            auto k_global = wave_group * GROUP_SIZE + wave_half * 16 + lane_k;

            // operand_t has size GROUP_UINT4 uint4 slots: 1 for fp16/bf16,
            // 2 for tf32. weights_lds in operand_t units so the index step
            // along khw is GROUP_UINT4 / UINT4_PER_LANE_FRAGMENT.
            constexpr int LANE_FRAGS_PER_GROUP = GROUP_UINT4 / UINT4_PER_LANE_FRAGMENT;
            const operand_t* weights_lds       = reinterpret_cast<const operand_t*>(lds_buf);
            for(int khw = 0; khw < cfg.kh * cfg.kw; khw++)
            {
                operand_t w_raw =
                    *(weights_lds + k_global * (cfg.kh * cfg.kw * LANE_FRAGS_PER_GROUP) +
                      khw * LANE_FRAGS_PER_GROUP + lane_c8_w);
                weights_reg[khw] = Traits::to_mfma_operand(w_raw);
            }
        }
    }
    ...
    const int lane_q_in   = OperandLayout::outer(lane);
    const int lane_c8_in  = OperandLayout::inner(lane) / 8;
    const int lane_q_out  = ResultLayout::outer(lane);
    const int lane_c4_out = ResultLayout::inner(lane) / 4;

    // Map threads to output addresses for transferring through LDS to global
    // memory. Output per lane is 4 elements: uint2 (fp16/bf16) or uint4 (tf32).
    ...
    if(store_active)
    {
        ...
        if constexpr(Traits::needs_lds_pack)
        {
            // fp16/bf16: naive modulo works because BLOCK_C_UINT4 divides
            // tid evenly within each row across the STORE_VECS range.
        ...
        else
        {
            ...
            // vanish for all C4, so use the swizzle inverses.
            col     = Sw::x(tid);
            c_uint4 = Sw::c4(tid);
        }
    ...
    // Each lane reads operand_t (= UINT4_PER_LANE_FRAGMENT consecutive uint4s)
    // per filter column S. For fp16/bf16 this is one uint4 per lane; for tf32
    // it is two contiguous uint4 read together as fp32x8.
    int input_lds_offsets[cfg.kw];
    bool input_lds_mask[cfg.kw]; // only meaningful for dilation > 1
    static_for<cfg.kw>([&]<int S>() {
        if constexpr(cfg.dilation > 1)
        {
            int dilated_global = (block_q - px) + lane_q_in + S;
            ...
            input_lds_offsets[S] = Sw::offset_uint4(
                lds_col, wave_group * GROUP_UINT4 + lane_c8_in * UINT4_PER_LANE_FRAGMENT);
            input_lds_mask[S] = is_real;
        }
        else
        {
            input_lds_offsets[S] = Sw::offset_uint4(
                lane_q_in + S, wave_group * GROUP_UINT4 + lane_c8_in * UINT4_PER_LANE_FRAGMENT);
            input_lds_mask[S] = true;
        }
    });

    // Pre-compute the output LDS swizzle offset (thread-constant). Single
    // write per lane: wave_half selects which 16-channel half. The output
    // stride along C is GROUP_SIZE/4 (in lds_unit_t units, where lds_unit_t
    // is uint2 for fp16/bf16 = 4 ch; uint4 for tf32 = 4 ch).
    constexpr int LANE_GROUP_C4 = GROUP_SIZE / 4; // 8 lds_unit_t strips per group
    int output_lds_offset;
    bool output_lane_active;
    auto output_offset_fn = [](int x, int c4) constexpr -> int {
        if constexpr(Traits::needs_lds_pack)
    ...
    if constexpr(cfg.stride == 2)
    {
        output_lane_active = (lane_q_out % 2 == 0);
        output_lds_offset  = output_offset_fn(
            lane_q_out / 2, wave_group * LANE_GROUP_C4 + wave_half * 4 + lane_c4_out);
    }
    else
    {
        output_lane_active = true;
        output_lds_offset =
            output_offset_fn(lane_q_out, wave_group * LANE_GROUP_C4 + wave_half * 4 + lane_c4_out);
    }
    ...
    for(int y_base = 0; y_base + cfg.kh <= hi_eff; y_base += cfg.kh)
    {
        static_for<cfg.kh>([&]<int Y_LOCAL>() {
            ...
            static_for<cfg.kw>([&]<int S>() {
                const operand_t* lds_ptr = reinterpret_cast<const operand_t*>(
                    input_lds + toc * INPUT_LDS_BUFFER_UINT4 + input_lds_offsets[S]);
                operand_t input_raw = *lds_ptr;

                if constexpr(cfg.dilation > 1)
                {
                    bool row_is_real = (y % cfg.dilation == 0);
                    if(!row_is_real || !input_lds_mask[S])
                        input_raw = operand_t{0, 0, 0, 0, 0, 0, 0, 0};
                }
                ...
                static_for<cfg.kh>([&]<int R>() {
                    ...
                    if constexpr(cfg.direction == Direction::Dgrad)
                        acc[p_idx] =
                            mfma_16x16x32(weights_reg[(cfg.kh - 1 - R) * cfg.kw + (cfg.kw - 1 - S)],
                                          input_reg,
                                          acc[p_idx]);
                    else
                        acc[p_idx] =
                            mfma_16x16x32(weights_reg[R * cfg.kw + S], input_reg, acc[p_idx]);
                });
            });
...
// Compute grouped convolution with stride 1 or 2 and group size 32.
//
...
template <Config cfg, DataType DT>
__global__ void conv2d_grouped_32c_nhwc_cdna4(const ToType<DT>* __restrict__ in,
                                              const ToType<DT>* __restrict__ wei,
                                              double alpha,
                                              double beta,
                                              ToType<DT>* __restrict__ out,
                                              ...
                                              int px)
{
    ...
    {
        conv2d_grouped_32c_cdna4_nhwc_impl<cfg, DT>(in,
                                                    wei,
                                                    alpha,
                                                    beta,
                                                    out,
...
void launch_impl(const LaunchParams& lp,
                 ...
                 hipStream_t stream)
{
    // Bridge par.input_type (runtime) to DT (compile-time) for kernel instantiation.
    auto typed_launch = [&]<DataType DT>() {
        using dtype = ToType<DT>;
        auto view   = SizeView<cfg.direction>(par);
        conv2d_grouped_32c_nhwc_cdna4<cfg, DT>
...
class Grouped_32C_ConvKernel : public GroupedConvKernel
{
public:
    constexpr Grouped_32C_ConvKernel(const Config& cfg, LaunchFn launch_fn)
...
HIPCONV_DEFINE_KERNEL_TABLE(Grouped_32C_ConvKernel);
...
HIPCONV_EXPORT_KERNEL_TABLE(grouped_32c_cdna4_kernels, hipconv::cdna4::grouped_32c);
#endif
