...
**Overlapping IDs at load.** **`HeuristicPluginManager`** **must** reject a heuristic module whose **`hipdnnHeuristicGetPolicyId`** duplicates an **`int64_t`** already claimed by another **accepted** heuristic plugin or built-in ([§10.1](#101-heuristicpluginmanager), [§11](#11-versioning-and-compatibility-checks)). The built-in **`StaticOrdering`** is registered first, so a third-party module trying to claim the same ID will be rejected. This is **stricter than the engine side**, which drops only the duplicate engine — a module that cannot register its policy has nothing left to contribute.
...
- **`EnginePluginManager`** — searches paths, loads `.so` files, owns **`EnginePlugin`** instances. **`validateBeforeAdding`** checks the API major and rejects a plugin whole for no engines or a duplicate ID **within itself**; **`actionAfterAdding`** applies the per-engine rules (ID must equal the hash of the reported name; one ID admits one engine), dropping only the offending engine.
...
2. **Policy ID uniqueness:** Two loaded heuristic modules **must not** return the same **`int64_t`** from **`hipdnnHeuristicGetPolicyId`**. Enforce with an “insert into a set, throw on duplicate” pattern. Engines use the same insert-is-the-check idea in **`actionAfterAdding`**, but log and drop the engine instead of throwing ([§5.3.1](#531-well-known-policy-names-and-ids)).
