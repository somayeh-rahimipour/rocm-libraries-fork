...
"""
bert_block_ab.py -- run a stack of BERT-style **bidirectional encoder** blocks and A/B the
forward native vs the hipdnn_torch injection layer.

The point of this entry: it is the **non-causal** counterpart to ``gpt2_block_ab.py``. Same
op families (gemm, SDPA, LayerNorm, GELU) but the attention is full/bidirectional
(``is_causal=False``, no mask) and the norms are **post-norm** (add-then-LayerNorm), so it
probes whether dropping causality changes what the SDPA path will serve. Geometry defaults to
BERT-base (dim=768, 12 heads, head_dim=64, mlp=3072).

Self-contained (no ``transformers``); random weights are fine for routing/coverage. Always
run under ``HIPDNN_TORCH_SELECT=default`` with all providers co-loaded.

    HIPDNN_TORCH_SELECT=default HIPDNN_TORCH_PROVIDER_SOS=<all .dll> \
        python samples/bert_block_ab.py --ops linear,layernorm,sdpa,gelu

Knobs: BERT_DIM BERT_HEADS BERT_MLP BERT_SEQ BERT_LAYERS BERT_B BERT_WARMUP BERT_ITERS. Set
...
def build_bert(torch, dim, heads, mlp, seq, layers, device, dtype):
    """A canonical BERT encoder stack (bidirectional MHA, post-norm, GELU MLP)."""
    nn = torch.nn
    ...
    class Block(nn.Module):
        ...
        def forward(self, x):
            b, s, _ = x.shape
            qkv = self.qkv(x).view(b, s, 3, heads, head_dim).permute(2, 0, 3, 1, 4)
            ...
            x = self.ln1(x + self.proj(a))  # post-norm
            x = self.ln2(x + self.out(F.gelu(self.fc(x))))
            return x
    ...
    return BERT().to(device=device, dtype=dtype).eval()
...
def main() -> int:
    ...
    dim = int(os.environ.get("BERT_DIM", "768"))
    heads = int(os.environ.get("BERT_HEADS", "12"))
    mlp = int(os.environ.get("BERT_MLP", "3072"))
    seq = int(os.environ.get("BERT_SEQ", "512"))
    layers = int(os.environ.get("BERT_LAYERS", "6"))
    batch = int(os.environ.get("BERT_B", "1"))
    warmup = int(os.environ.get("BERT_WARMUP", "3"))
    iters = int(os.environ.get("BERT_ITERS", "10"))
